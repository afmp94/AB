class ApisController < ApplicationController
  ...
  CLIENT_ID = Rails.application.secrets.social[:google_client_id]
  CLIENT_SECRET = Rails.application.secrets.social[:google_client_secret]
  CLIENT_SCOPE = "https://www.googleapis.com/auth/userinfo.profile "\
                 "https://www.googleapis.com/auth/userinfo.email "\
                 "https://www.googleapis.com/auth/calendar.readonly".freeze
  ...
  def create
    ...
    if user_tokens["error"].nil?
      ...
      # if(user == nil)
      #   user = User.create(:email => user_info['email'], :first_name => user_info['given_name'],
      #                      :last_name => user_info['family_name'], :role => 'User',
      #                      :uid => user_info['id'], :refresh_token => user_tokens['refresh_token'],
      #                      :access_token => user_tokens['access_token'],
      #                      :expires => user_tokens['expires'])
      ...
      if authorization.nil?
        Authorization.create(
          ...
          access_token: user_tokens["access_token"],
          refresh_expires: user_tokens["expires"]
        )
      else
      ...
      flash[:notice] = "Google authentication is completed successfully"
    else
      flash[:error] = "Something went wrong! Please contact support for help"
    end
