class UserSettings::LeadGroupsController < ApplicationController
  ...
  before_action :load_lead_group, except: %i(create index new)
  ...
  def add_user_to_lead_group
    ...
    flash_message = if new_agent_for_lead_group.blank?
                      { danger: "Agent with email #{lead_group_agent_params[:email]} not found!" }
                    elsif @lead_group.users.include?(new_agent_for_lead_group)
                      { danger: "Agent #{new_agent_for_lead_group.full_name} is already in this lead group." }
                    else
                      @lead_group.users << new_agent_for_lead_group
                      { notice: "Added agent #{new_agent_for_lead_group.full_name} to the lead group." }
                    end

    redirect_to([:user_settings, @lead_group], **flash_message)
  end
