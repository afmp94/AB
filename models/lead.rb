...
class Lead < ActiveRecord::Base
  ...
  has_many :action_plan_memberships, dependent: :nullify
  has_many :coaching_messages, as: :messageable, dependent: :destroy
  has_many :key_person_logs, dependent: :destroy
  has_one :loop, dependent: :destroy

  accepts_nested_attributes_for :contracts, :contact, :key_people,
                                :properties, allow_destroy: true

  validates :contact, presence: true
  validate :validate_key_person
  ...
  after_create :create_task_for_missing_information
  before_save :update_contract_status
  before_save :update_contract_referral_fee
  before_save :set_name
  before_save :calculate_displayed_values
  ...
  before_save :handle_referring_contact_id
  ...
  LEAD_SOURCES = ["trulia", "zillow", "realtor", "moxie", "cinc"].freeze
  LEAD_TYPES = [
    ["Referral",           0],
    ["Internet",           1],
    ["Listed Property",    2],
  ...
  scope :client_long_term_prospect_due, -> do
    where("status = 8 AND long_term_prospect_remind_me_at < ?", Time.current.beginning_of_day)
  end
  ...
  scope :closed_buyer_leads_after_date, ->(date) do
    where("displayed_closing_date_at >= ? AND status = 4 AND client_type = 'Buyer'", date)
  end
  scope :closed_leads_after_date, ->(date) { where("displayed_closing_date_at >= ? AND status = 4", date) }
  scope :closed_leads_for_year, ->(year) do
    where("date_part('year', displayed_closing_date_at) = ? AND status = 4", year)
  end
  scope :closed_seller_leads_after_date, ->(date) do
    where("displayed_closing_date_at >= ? AND status = 4 AND client_type = 'Seller'", date)
  end
  scope :contact_not_made, -> { where("contacted_status <> 3 ") }
  scope :contacted_leads_with_dates, -> do
    where("contacted_status <> 0 AND incoming_lead_at is not null AND attempted_contact_at is not null")
  end
  scope :created_after_date, ->(date) { where("created_at >= ?", date) }
  scope :incoming, -> { where(status: 0).order("created_at desc") }
  ...
  scope :leads_claimed_by_other_user, ->(user) do
    where("(leads.created_by_user_id = ?) AND (leads.user_id != ?)", user, user)
  end
  ...
  scope :leads_responsible_for, ->(user_ids) do
    lead_status.where("(leads.user_id IN (:ids)) OR (leads.created_by_user_id IN (:ids) AND leads.state != 'claimed')",
                      ids: user_ids)
  end
  ...
  scope :open_clients_and_leads, -> { where("status in (0, 1, 2, 3, 5)") }
  ...
  scope :owned_or_created_by_user, ->(user) do
    where("(leads.user_id = ?) OR (leads.created_by_user_id = ?)", user, user)
  end
  scope :search_name, ->(search_term) do
    joins(:contact).
      where(
        "contacts.first_name ILIKE :search_term OR contacts.last_name ILIKE :search_term",
        search_term: "%#{search_term.downcase}%"
      )
  end
  scope :sellers, -> { where("client_type = 'Seller'") }
  scope :unclaimed_leads_responsible_for, ->(user) do
    lead_status.
      where(
        "(leads.user_id = ? AND leads.state != 'claimed') OR "\
        "(leads.created_by_user_id = ? AND leads.state != 'claimed')",
        user,
        user
      )
  end
  scope :updatable_for_commission_data, -> do
    where(displayed_broker_commission_custom: false).where.not(status: [4, 6, 7])
  end
  ...
  scope :last_year_leads, -> do
    where(displayed_closing_date_at: Time.current.last_year.beginning_of_year..Time.current.last_year.end_of_year)
  end
  ...
  extend_recent_activities_for(
    ...
    contracts: {
      condition: ->(contact) do
        {
          recipient_id: contact.id,
          recipient_type: contact.class.name,
      ...
      condition: ->(lead) do
        {
          ...
          recipient_type: lead.class.name,
          key: ["key_person.create", "key_person.destroy"]
        }
      end
    },
  )
  ...
  def self.incoming_at_during_last_year
    self.incoming_at_during_range(
      start_time: (Time.current - 1.year).beginning_of_year,
      end_time: (Time.current - 1.year).end_of_year
    )
  end
  ...
  def calculate_displayed_values
    LeadUpdateDisplayedFieldsService.new(self).calculate_all
  end
  ...
  def set_name
    LeadAttributesCalculationService.new(self).set_lead_name
  end
  ...
  def lead_type_to_s
    lead_type.present? ? lead_type.name : ""
  end
  ...
  def set_long_term_prospect_remind_me_at(datetime)
    if self.status == 8
      self.long_term_prospect_remind_me_at = datetime.presence || 1.month.from_now
    end
  end
  ...
  def update_contract_status
    if contract = (accepted_listing_contract || accepted_buyer_contract)
      ...
      Rails.logger.info "[LEAD.update_contract_status] Changed: #{self.changes.keys}"
      Rails.logger.info "[LEAD.update_contract_status] Status: #{status}"
      if self.changes.keys.include?("status")
        if status == 3
  ...
  def broker_commission_defined?
    if displayed_broker_commission_custom
      displayed_broker_commission_type.present? &&
        (displayed_broker_commission_percentage.present? ||
         displayed_broker_commission_fee.present?)
    else
      false
    end
  end

  def active_and_has_an_accepted_contract?
    status_to_s == STATUSES[2][0] &&
      (accepted_listing_contract.present? || accepted_buyer_contract.present?)
  end
  ...
  def handle_referring_contact_id
    if lead_type && lead_type.is_existing_relationship_type?
      self.referring_contact_id = self.contact_id
    end
  end

  def create_task_for_missing_information
    @missing_key_information = nil

    if require_task
      task_message = "Update key Information (#{@missing_key_information.join(', ')}) - may impact your Income Goals"

      tasks.create!(subject: task_message,
                    user_id: user.id,
  ...
  # rubocop:disable Metrics/AbcSize
  def require_task
    if listing? && (Lead::STATUSES[status][0] == Lead::STATUSES[2][0])
      @missing_key_information = add_missing_information_for_listing_lead

      listing_property&.listing_expires_at.nil? || price_or_net_commission_nil?
    elsif pending?
      @missing_key_information = add_missing_information_for_pending_lead

      price_or_net_commission_nil?
    elsif closed?
      @missing_key_information = add_missing_information_for_closed_lead

      displayed_closing_date_at.nil? || price_or_net_commission_nil?
    else
      false
    end
  end
  ...
  def price_or_net_commission_nil?
    displayed_price.nil? || displayed_net_commission.nil?
  end

  def add_missing_information_for_listing_lead
    missing_key_information = []

    missing_key_information << "Listing expires at" if listing_property&.listing_expires_at.nil?
    missing_key_information << "Price" if displayed_price.nil?
    missing_key_information << "Commission" if displayed_net_commission.nil?

    missing_key_information
  end

  def add_missing_information_for_pending_lead
    missing_key_information = []
  ...
  def add_missing_information_for_closed_lead
    missing_key_information = []

    missing_key_information << "Closing date" if displayed_closing_date_at.nil?
    missing_key_information << "Price" if displayed_price.nil?
    missing_key_information << "Commission" if displayed_net_commission.nil?
  ...
  def validate_key_person
    # This code is little hack.
    # NOTE: Need a better way to handle this.

    key_people.select(&:new_record?).each do |key_person|
      next if key_person.valid?

      key_person.errors.full_messages.each do |msg|
        errors[:base] << msg
      end
    end
  end
