class NylasApi::Message
  ...
  def find_by_email_addresses(email_addresses)
    if account && email_addresses.present?
      account.messages.where(any_email: email_addresses.join(","), offset: 0, limit: 10)
    end
  end
  ...
  def total_count(from:, to:)
    account.messages.where(from: from, to: to, view: "count").execute[:count]
  end

  def past_year_count(from:, to:)
    account.messages.where(from: from, to: to, received_after: past_year, view: "count").execute[:count]
  end
