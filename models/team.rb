...
class Team < ActiveRecord::Base
  ...
  def self.add_teammate(user_email, team_owner)
    ...
    if new_teammate_not_found?
      { danger: "Agent with email #{user_email} not found!" }
    elsif already_teammates?
      { danger: "Agent #{@new_teammate.full_name} is already part of your team." }
    elsif new_teammate_same_as_user?
      { danger: "You can't add yourself as your teammate." }
    elsif new_teammate_has_already_confirmed_user?
      confirm_new_teammate
      { notice: "#{@new_teammate.full_name} and you are now teammates." }
    else
      invite_new_teammate_to_join_team
      { notice: "Invited agent #{@new_teammate.full_name} to join your team." }
    end
  end
