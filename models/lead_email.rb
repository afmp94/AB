...
class LeadEmail < ActiveRecord::Base
  ...
  scope :failed_parsing, -> do
    where("parsing_attempts < ? AND importing_state = ?", MAX_PARSING_ATTEMPTS, "parsing_error")
  end
  ...
  def parsing_attempted!
    if parsing_attempts <= MAX_PARSING_ATTEMPTS
      self.increment!(:parsing_attempts)
    end
  end
