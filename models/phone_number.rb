...
class PhoneNumber < ActiveRecord::Base
  ...
  after_destroy :update_contact_primary_phone_number, if: :primary?
  after_destroy :set_contact_phone_number
  ...
  def unset_old_primary_phone_number
    ...
    old_phone_numbers.update_all(primary: false, updated_at: Time.current)
  end
  ...
  def update_contact_primary_phone_number
    if owner.is_a?(Contact)
      contact = self.owner
      phone_number = contact.phone_numbers.reload.first
      phone_number.update_columns(primary: true) if phone_number.present?
    end
  end
