class Invoice
  ...
  delegate(
    :name, :billing_organization, :billing_address, :billing_address_2,
    :billing_city, :billing_state, :billing_zip_code, :billing_country, :email,
    to: :user, prefix: true
  )
  ...
  def initialize(invoice_id_or_invoice_object)
    @stripe_invoice = if invoice_id_or_invoice_object.is_a? String
                        retrieve_stripe_invoice(invoice_id_or_invoice_object)
                      else
                        invoice_id_or_invoice_object
                      end
  end
