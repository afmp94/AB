...
class Task < ActiveRecord::Base
  ...
  scope :upcoming, -> { where("due_date_at > ? OR due_date_at IS NULL", Time.zone.now.end_of_day) }
  ...
  tracked(
    ...
    params: { changes: :saved_changes, name: :subject },
    on: { update: proc { |model, _| model.saved_changes.keys != ["updated_at"] } }
  )

  def self.for_client_testimonial(user, lead, contract)
    due_date_at = if contract.closing_date_at.present?
                    [(contract.closing_date_at - 7.days), Time.current].max
                  else
                    Time.current
                  end

    task = Task.new.tap do |_task|
      _task.subject        = "Send testimonial to #{lead.name}"
      _task.due_date_at    = due_date_at.end_of_day
      _task.assigned_to_id = user.id
      _task.user_id        = user.id
      _task.taskable_id    = lead.id
      _task.taskable_type  = lead.class.name
    end
  ...
  def set_incomplete_lead_task_counter_if_changed
    if self.taskable_type == "Lead" && self.taskable_id && self.saved_changes.keys.include?("completed")
      self.taskable.incomplete_tasks_count = self.taskable.tasks.not_completed.count
    end
  end
  ...
  def teamtask_for?(user)
    user_id == user.id && assigned_to_id != user.id
  end
