...
class Contract < ActiveRecord::Base
  ...
  validate :limit_one_accepted_contract
  validates :offer_price, presence: { message: "Contract must have a price" }

  after_destroy :update_lead_displayed_values
  after_save :update_lead_displayed_values
  before_destroy :reset_property_status
  before_destroy :reset_lead_status
  before_save :update_closing_price
  before_save :update_lead_referral_fees
  ...
  def limit_one_accepted_contract
    if accepted_status? && other_accepted_contract?
      errors.add(:status, "You already have an accepted contract for this client.")
    end
  end

  def other_accepted_contract?
    accepted_contracts = lead.contracts.accepted
    if accepted_contracts.present?
      if accepted_contracts.include?(self)
        false
      else
        true
      end
  ...
  def update_lead_displayed_values
    Rails.logger.info "[update_lead_displayed_values] contract: #{id}"
    Rails.logger.info "  Lead: #{self.lead.id}"
    Rails.logger.info "  Lead persisted?: #{self.lead.persisted?}"
    Rails.logger.info "  Lead destroyed?: #{self.lead.destroyed?}"
