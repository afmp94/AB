...
class EmailAddress < ActiveRecord::Base
  ...
  after_destroy :update_contact_primary_email_address, if: :primary?
  after_destroy :set_contact_email
  ...
  def update_contact_primary_email_address
    if owner.is_a?(Contact)
      contact = self.owner
      email_address = contact.email_addresses.reload.first
      email_address.update_columns(primary: true) if email_address.present?
    end
  end
