...
class ContactActivity < ActiveRecord::Base
  ...
  validates :user_id, :contact_id, :activity_type, presence: true
  validates :completed_at, presence: true, if: -> { custom_time == CUSTOM_TIME_TYPES[:specify_date] }
  validate :completed_at_should_be_in_past

  after_create :set_last_activity_at, :increase_wkly_and_daily_counter
  before_save :handle_completed_at_for_custom_time

  include PublicActivity::Model
  tracked(
    owner: proc { |controller, _contact_activity| controller&.current_user },
    ...
    on: {
      update: proc do |model, _controller|
        model.saved_changes.keys != ["updated_at"]
      end
    }
  )
  ...
  def required_changes
    return saved_changes if saved_changes.blank?

    saved_changes.keep_if do |key, _|
  ...
  def handle_completed_at_for_custom_time
    self.completed_at = Time.current if custom_time == CUSTOM_TIME_TYPES[:just_now]
  end

  def completed_at_should_be_in_past
    if completed_at.present? && completed_at > Time.current
      errors.add(:completed_at, "should be in past")
    end
  end
