...
class CsvParsingService
  ...
  def row_parse_error(smart_row, error)
    Rails.logger.error "[CSV.parsing] Row parse error: #{error.inspect}"
    csv_file.invalid_records.create!(
      original_row: smart_row.row.to_csv,
      contact_errors: contact.try(:errors).try(:full_messages) || [error.inspect]
    )
  end
