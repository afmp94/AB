class ConnectWithContactService
  ...
  def perform
    ...
    contacts.each do |contact|
      if contact&.user&.subscription.present?
        if contact.user.subscription.deactivated_on.nil?
          user = contact.user
  ...
  def send_sms user, contact
    ...
    SmsService.new.dispatch(
      ...
      payload: payload
    ) # to_user: true, user_id: user.id, quiet_hours: user.lead_setting.quiet_hours
  end
